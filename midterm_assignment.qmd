...
**AI Declaration**:
 delete this line and state your AI usage here. 
:::
...
- Set the random seed to 42. The code is included. This is for reproducibility and must be executed for consistent reproduction of the same outputs.
...
- After this, remove all rows where the value of `Ozone` is more than 1.5 standard deviations __above__ the mean `Ozone` level in our dataset. Name this new dataframe `aq_clean`.
...
- Add a new column titled `Wind_kmh` which contains the wind speed in kilometres per hour using the `mutate()` function from the `dplyr` package in the `tidyverse.` The data is currently in miles per hour (1 mph = 1.609 km/h).

- Use `group_by()` and `summarise()` to calculate the average `Temp` and `Wind_kmh` per month.

- Which month has the highest average wind speed? What is this value?

- Which month has the lowest average temperature? What is this value?
...
You have cleaned air‑quality data in a data frame `aq_clean,` and want to quantify the variability of the sample mean of `Ozone.` In this subquestion, we will use a for-loop to get a bootstrapped standard error for the mean of the variable Ozone.

- Set the random seed to 42 again. The code is included, so that everyone's bootstrap draws are identical.

- Use the `mean()` function to calculate the sample mean of `aq_clean$Ozone`.

- Assign `2000` to a numeric vector of length 1 named `n_boot`.
...
- In a for‑loop over i in 1:`n_boot`:
    ...
    - Draw a bootstrap sample of size `length(aq_clean$Ozone)` using the function `sample()` with replacement from `aq_clean$Ozone`

    - Compute its mean and store it in `boot_means[i]`

- Finally, use `boot_means` and the  `sd()` function to estimate the bootstrapped standard error for the mean of the variable `Ozone`.

- Report both the sample mean (calculated in step 2) and its bootstrapped standard error.

```{r}
# Set seed again so bootstrap draws are identical for everyone
set.seed(42)
...
Use an if/else control flow to label days by whether they exceed the median `Temp` level.

- Calculate the median `Temp` using the current `aq_clean` dataframe.

- Create a new categorical variable in `aq_clean` titled `Temp_Status` with the label "Hot" if the row's Temp exceeds the median, and "Mild" if not. You must use `ifelse()` for this.

- Calculate how many "Hot" days there are and report this number.
...
Imagine you want to examine how exercise habits affect resting heart rate among 400 adults. However, the distribution of weekly exercise time is skewed: most adults exercise little, but a few exercise a lot. The resting heart rates are generated using the following model:
$$
\text{resting\_hr} = 72 - 1.5 \times \text{exercise\_hours} + \epsilon
$$

where  $\epsilon \sim \mathcal{N}(0,6)$ represents random noise.

**2.1 Simulate the data:**

- Set the random seed to 2026. 

- Generate a vector called `exercise_hours` for 400 adults using an exponential distribution, scaled so that most adults exercise between 0 and 15 hours per week.

- Generate a vector called `group` with two unique values: "gym" and "home" (200 adults each), randomly assigned.

- Generate `resting_hr` using the data generating process defined above, adding random noise $\epsilon$ drawn from a normal distribution with mean 0 and standard deviation 6.

- Create a data frame called `fitness_data` with the three variables.
...
- Create a scatter plot of `resting_hr` versus `exercise_hours`, with points colored by `group`.
...
Describe the plot. Based on the model we used to create resting_hr, why is it not surprising that there is no visible difference in resting heart rates between adults who train at a gym and those who train at home?
...
For question 3, you will be working with a dataset about cats in an animal shelter, waiting to be adopted. The dataset contains some information about cats' medical history and appearance and a variable `adoption_speed` that indicates how quickly the cat was adopted after being put up for adoption. The higher the value of `adoption_speed`, the quicker the cat was adopted.
...
| dewormed        | Whether the cat is dewormed (1 = Yes, 0 = No)                    |        
| vaccinated      | Whether the cat is vaccinated (1 = Yes, 0 = No)                  |
| fur_length      | Length of the cat's fur in centimeters                           |
| weight          | Weight of the cat in kilograms                                   |
...
For question 4 you will continue working with the cats dataset from the previous question, but now you will estimate the causal effect of vaccination on adoption speed.
